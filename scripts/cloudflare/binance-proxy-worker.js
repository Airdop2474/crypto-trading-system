 ...
 * Uses new WebSocket() for upstream (avoids fetch WS upgrade issues).
 * Flow: accept client WS -> new WebSocket(upstream) -> bidirectional pipe.
 */
async function handleWebSocket(request, wsSubPath) {
  const upstreamUrl = `wss://stream.binance.com:9443/ws/${wsSubPath}`;

  // 1. Create client WebSocket pair
  ...
  // 3. Connect to upstream via WebSocket constructor
  //    (fetch(wss://) returns 500 due to header/upgrade issues;
  //     WebSocket constructor handles the handshake natively)
  ...
  upstreamWs.addEventListener("error", () => {
    try { server.close(1011, "upstream connection failed"); } catch (_) {}
  });
