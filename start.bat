...
REM --- OpenAI API（选填） ---
findstr /C:"OPENAI_API_KEY=your_openai" .env >nul 2>nul
if not errorlevel 1 set NEED_CONFIG=1
...
if "!NEED_CONFIG!"=="1" (
    ...
    echo.
    echo   [4] OpenAI API Key（用于策略进化 AI 解读，获取: https://platform.openai.com/api-keys）
    set /p "OA_KEY=       Key（回车跳过）: "
    if not "!OA_KEY!"=="" (
        python -c "import re; p='.env'; t=open(p,encoding='utf-8').read(); t=re.sub(r'OPENAI_API_KEY=.*','OPENAI_API_KEY=!OA_KEY!',t); open(p,'w',encoding='utf-8').write(t)"
        echo       ^> OPENAI_API_KEY 已设置
    ) else (
        echo       ^> 已跳过（将使用本地规则回退解读）
    )
