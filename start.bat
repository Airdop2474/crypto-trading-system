...
REM ========================================================================
REM  Step 5: 策略淘汰评估（可选，基于已有 daemon 运行数据）
REM ========================================================================
echo [5/7] 策略淘汰评估 ...
echo   基于 daemon 运行数据评估所有 active 策略，自动归档低分策略。
echo   首次启动（无运行数据）可直接跳过。
echo.
echo   1 = 仅查看评估报告（dry-run，推荐）
echo   2 = 评估并实际归档低分策略
echo   3 = 跳过（默认）
set /p "ELIM_CHOICE=  请选择 (1/2/3，回车=3): "

if "!ELIM_CHOICE!"=="1" goto elim_dryrun
if "!ELIM_CHOICE!"=="2" goto elim_archive
goto elim_skip

:elim_dryrun
echo.
python scripts\strategy_eliminator.py --dry-run
echo.
echo   报告已生成（见上方输出）。本次仅查看，未实际归档。
echo   如需归档，可稍后运行: python scripts\strategy_eliminator.py
pause
goto elim_done
...
python scripts\strategy_eliminator.py --dry-run
echo.
set /p "ELIM_CONFIRM=  输入 y 确认归档（其他键跳过）: "
if /i "!ELIM_CONFIRM!"=="y" (
    python scripts\strategy_eliminator.py
    echo.
    echo   归档完成。归档的策略不会自动启动（可在前端策略页手动恢复）。
) else (
    echo   已跳过实际归档。
)
pause
