...
运行工具：`python scripts/run_paper_trading_daemon.py --days 60`（实时，每根 bar
落库 + 跨日出日报，崩溃可凭检查点续跑）。下列为**时间/人工轨道**，需真实跑满
60 天并人工核验后才能勾选——脚本只是工具，不代表已完成。

- [ ] **连续运行 60 天**，无系统故障
- [ ] 每日摘要报告完整（60份）  ← 守护进程产 `data/reports/paper/daily/daily_*.md`
- [ ] 所有信号和订单可追溯
- [ ] 风控触发记录完整
- [ ] 模拟盘与回测偏差可解释
- [ ] 系统暂停和恢复机制测试通过  ← 机制已自动化测试（resume 标志），实跑中再核验

### 2. 风控测试

自动化验证：`python scripts/verify_risk_controls.py`（6/6 PASS，退出码 0）；
并纳入 CI：`tests/unit/test_verify_risk_controls.py`。
